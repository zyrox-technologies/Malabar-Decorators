...
import { ArrowUpRight, UtensilsCrossed, ShieldCheck, Flame, MessageCircle } from "lucide-react";
...
export default function MenuHook() {
  ...
  return (
    <section className="py-space-3xl md:py-space-4xl bg-[#FAF6EE] border-t border-b border-[#790504]/10 relative overflow-hidden select-none" id="menu-hook">
      ...
      <div className="max-w-max-width mx-auto px-gutter-mobile md:px-gutter-tablet lg:px-gutter-desktop relative z-10">
        ...
        {/* 3 Featured Menu Cards Grid */}
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 items-stretch mb-14">
          {menuHighlights.map((dish, index) => (
            <Reveal key={dish.id} delay={index * 0.1} from="up" className="h-full flex flex-col">
              <div className="group flex flex-col bg-surface border border-surface-variant/80 rounded-2xl overflow-hidden transition-all duration-500 hover:shadow-xl hover:border-secondary/40 hover:-translate-y-1.5 h-full">
                {/* Dish Image */}
                ...
                <div className="p-6 sm:p-7 flex flex-col flex-1 justify-between bg-surface">
                  <div className="space-y-3">
                    <h3 className="font-headline-md text-xl md:text-2xl text-on-surface font-serif group-hover:text-primary transition-colors">
                      {dish.title}
                    </h3>
                  ...
                  <div className="pt-6 mt-6 border-t border-surface-variant/60">
                    <Link
                      href="/menu"
                    ...
                    >
                      <span>Explore Dish Selection</span>
