...
function ServiceBadgeIcon({ type }: { type: ServiceItem["iconType"] }) {
  switch (type) {
    ...
    case "reception":
      // Royal crown / stage architectural motif
      return (
        <svg
          className="w-6 h-6 text-secondary"
          viewBox="0 0 24 24"
  ...
  return (
    ...
    >
      <div className="max-w-max-width mx-auto px-gutter-mobile md:px-gutter-tablet lg:px-gutter-desktop">
        {/* Section Header with Classical Ornament */}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 lg:gap-8">
          {servicesData.items.map((service) => (
            <div
              key={service.id}
              className="bg-white rounded-2xl border border-surface-variant/70 
                         shadow-[0_4px_25px_rgba(0,0,0,0.03)] 
                         hover:shadow-[0_16px_35px_rgba(171,54,0,0.08)] 
                         transition-all duration-300 flex flex-col overflow-hidden group hover:-translate-y-1"
            >
              ...
              <div className="p-4 pb-0 relative">
                <div className="relative w-full aspect-[16/10] rounded-xl overflow-hidden bg-surface-container">
                  ...
                  <div
                    className="absolute -bottom-5 right-4 z-10 w-12 h-12 md:w-13 md:h-13 
                               rounded-full bg-white border border-surface-variant 
                               shadow-md flex items-center justify-center 
                               group-hover:scale-110 group-hover:bg-surface-container-low transition-transform duration-300"
                  >
                ...
                <div className="pt-4 border-t border-surface-variant/60 flex items-center justify-between mt-auto">
                  <Link
                    ...
                    className="text-xs uppercase tracking-wider font-semibold text-secondary 
                               inline-flex items-center gap-1.5 group-hover:gap-2.5 transition-all duration-300 group-hover:text-primary"
                  >
